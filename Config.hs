...
hook4InstHook pkg_descr lbi uhook flags = do 
  ...
  mapM_ (mkDynLibraries (tdir </> "pythia-pgs.orig" </> "libraries" </> "lhapdf" </> "lib") libPref) ["LHAPDF"]
  mapM_ (mkDynLibraries (tdir </> "pythia-pgs.orig" </> "libraries" </> "PGS4" </> "lib") libPref) [ "Fmcfio", "exthep", "pgslib", "stdhep", "tauola" ] 
  mapM_ (mkDynLibraries (tdir </> "pythia-pgs.orig" </> "libraries" </> "pylib" </> "lib") libPref) [ "pythiaext" ]
  return ()
...
config bInfo = do 
  ...
  let hbi = emptyBuildInfo { extraLibs = extraLibs buildinfo 
                           ...
                           , extraLibDirs = [tdir</>"pythia-pgs.orig"</>"libraries"</>"lhapdf"</>"lib"
