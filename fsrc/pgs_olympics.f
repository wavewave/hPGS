...
c--------------------------------------------------------
      ...
      subroutine mypgs( myargc, myargv ) 
       
      include 'pgs.inc'
      integer myargc,i
      character*40 myargv(20)

      logical pgs_next_event
      logical lhefset
      character *250 lhef_name
...
c      call pgs_user_args
      pgs_args(1)='--stdhep'
      pgs_args(2)='test.hep'
      pgs_args(3)='--nev'
      pgs_args(4)='10'
      pgs_args(5)='--L0'
      pgs_args(6)='testtest.lhco'
      pgs_args(7)=' '
      numarg = 6
c initialize PGS
...
c set up PGS *after* user initialization
      pgs_param_file='lhc.par'
      WRITE(*,*) 'pgs_param_file=',pgs_param_file
      
c      open (84,file=pgs_param_file,status='old')
c      WRITE(*,*) 'ha'
      call pgs_initialize
...
c
c      include 'herwig65.inc'  ! includes implicit none
c
